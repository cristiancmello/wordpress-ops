...
const { SwarmBasicCluster } = require("./components/SwarmBasicCluster/index");
const { Vpc } = require("./components/SwarmBasicCluster/Vpc");
const {
  SwarmManagerEntrypointFile,
} = require("./components/SwarmBasicCluster/SwarmManagerEntrypointFile");

const {
  SecurityGroup,
} = require("./components/SwarmBasicCluster/SecurityGroup");
...
class SwarmBasicClusterStack extends cdk.Stack {
  ...
  constructor(scope, id, props) {
    ...
    SwarmBasicCluster(this, "swarmBasicCluster", {
      vpc: Vpc(this, "swarmVpc", {
        cidr: "10.0.0.0/16",
        maxAzs: 2,
        natGateways: 0,
        subnetConfiguration: [
      ...
      managerEntrypointFile: SwarmManagerEntrypointFile(
        this,
        "swarmMasterEntrypointFile",
        {
          stationInput,
        }
      ),
    }).addComponent(
      SecurityGroup(this, "swarmNodeSecurityGroup", {
        groupName: `swarm-node-sg-${id}`,
        groupDescription: "Docker Swarm Node SG",
        securityGroupIngress: [
          {
    ...
    //     ],
    //   }),
    //   managerEntrypointFile: new EntrypointFile(
    //     this,
    //     "swarmNodeMasterEntrypointFile",
    //     {
    //       stationInput,
    //     }
    //   ),
    // })
    //   .addComponent(
    //     new SecurityGroup(this, "swarmNodeSecurityGroup", {
    //       groupName: `swarm-node-sg-${id}`,
    //       groupDescription: "Docker Swarm Node SG",
    //       securityGroupIngress: [
    //         {
    ...
    //   )
    //   .addComponent(
    //     new SecurityGroup(this, "myOtherSecurityGroup", {
    //       groupName: `swarm-node-sg-${id}-1`,
    //       groupDescription: "Docker Swarm Node SG",
    //     })
    //   );

    // const securityGroup = new SecurityGroup(this, "swarmNodeSecurityGroup", {
    //   groupName: `swarm-node-sg-${id}`,
    //   groupDescription: "Docker Swarm Node SG",
    //   securityGroupIngress: [
    //     {
    ...
    // new SwarmNode(this, "swarmNodeMaster", {
    //   vpc,
    // })
    //   .addComponent(securityGroup)
    //   .addComponent();

    /*

    <SwarmNode id="swarmNodeMaster" vpc={new Vpc...}>
      <SecurityGroup id="swarmNodeSecurityGroup" vpcId={this.props.vpc.vpcId} />
    </SwarmNode>
    */
